...
- **What changed.** `ah.port.mapping.ARTIFACT_PATH` -> `sleeve-mappings-v1.3.yaml`
  ...
  `port-v6-chosen-pe-gen`. Every live generated-plane world retires and is
  replaced by a byte-identical-except-identity successor in the `72x` chosen-PE
  block: `711` -> `721` (stress_1974_successor), `712` -> `722` (gulf_decade),
  `713` -> `723` (stress_1990_successor), and `604` -> `724` (stagflation_1974, the
  played generated preset — its tape moves under v1.3 exactly as the stress worlds'
  do, so the same rule applies), each carrying `parent_world_id` = the id it
  replaces and a provenance sentence; `711/712/713/604` join
  `ah.cli.RETIRED_WORLD_IDS` — scores under `port-v5-inflation-gen` and
  `port-v6-chosen-pe-gen` can never share a leaderboard row (alpha stamp + world
  fence, both). `app/fixtures/gen.bundle.gz` regenerated by its owning script
  (`scripts/gen_bundle_fixtures.py`, whose header prescribes a rebuild when the
  adapter changes): it now carries world `724`, v1.3 tapes and the v6 stamp.
...
- **The narration workbench (narr-04, TASK-wp4.2), 2026-08-15.** DN-9's
  ...
  `src/ah/narration/` - input adapter, event detection with severity, quarterly
  slate assembly, three voices behind one interface (Committee, columnists,
  House Economist), template banks as **data** (`templates/*.yaml`, so a copy
  change is not a code change), nine diagnostics panels, and the
  `build` / `compare` / `probe` CLI (`python -m ah.narration`).

  **Thirty-five open decisions existed and this build takes none of them.**
  Every tunable is read from `voices.yaml`; loading an `UNRESOLVED` key raises
  and the run fails with the whole list. `src/ah/narration/UNRESOLVED.md`
  records **58 open parameters** - name, where needed, what depends on it, 2-3
  candidates and the trade-off - generated from `ah.narration.params` so the
  document and the code that raises cannot drift. `voices.yaml` gained 18 keys
  the build needed and the skeleton lacked; none were filled.

  Because a workbench that can only fail measures nothing,
  `configs/voices-probe-unratified.yaml` is generated by one mechanical rule -
  every open key takes `candidates[0]` from `UNRESOLVED.md` - and every artifact
  it produces is stamped `config_status: PROBE-UNRATIFIED` and carries a banner.

  **First real-world build** (`narration_1974`, hier-flow-v1, seed 198001, one
  path): 463 events, 40 slates, 110 announcements. Three findings, all about the
  world rather than about the copy. **85 severity-3 events against a configured
  target band of 4-10** - the candidate cut-points are an order of magnitude out
  on a generated path, which is exactly the calibration DN-9 never did.
  **Meeting-to-meeting policy reversal frequency 0.60** (38 reversals across 63
  moves in 72 meetings), with steps up to 125bp - a diagnostic on the GENERATOR
  (DN-9 §C.6, referral N-q), measured here for the first time. And the smoothed
  anchor at the candidate rho=0.85 leaves epsilon spanning -175bp to +125bp
  (sd 60bp) where §C.6's worked example got +25/+5/+1bp - so rho is not
  transferable between worlds and must be fit, as N-p says.

  Nothing in `ah.core`/`ah.gen`/`ah.eval`/`ah.port` imports the package; an
  import-graph test asserts the direction. Run outputs live under a gitignored
  `runs/`.

- **The Wire Decade - the narration workbench's world (narr-03),
  2026-08-15.** `narration_1974.json` (world `...801`, its own fence, never
  a play or leaderboard block): the 1974-class declared regime shape
  generated by `hier-flow-v1`, the G2-promoted hierarchical generator,
  because the owner ruled the workbench's `l1_state` input stays REQUIRED
  rather than demoting the contract. Verified locally (the checkpoint is a
  gitignored campaign-2 artifact): `sample(world, 1, 198001)` returns
  (1, 120, 15) with SlowStateRecord (pi_star, r_star, g, v, credit_gap)
