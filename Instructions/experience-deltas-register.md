...
### §E1 — CLOSED 2026-08-12 (the finish-single-player plan, sp-01..sp-05)

Every not-landed item below landed: the per-sleeve commitment decision type
(sp-01, merge `2443225`; play accepts `{"action", "commitments"}` beside the
four actions, bounds validated); the t₀ pacing plan (sp-02, merge `01a0bf2`;
`next_plan_commitments` server-computed at the pointer, the lever pre-fill —
committing it unchanged IS holding to plan, and a touched lever is logged
verbatim in the window log, distinguishable from silence); the vintage stack
by age and trailing distributions (sp-05; session `vintage_nav` +
`trailing_distributions`, rendered in `PrivateMarkets.tsx`); the tutorial
consequence (sp-05; the `commit-primer` on the setup screen, shown to every
player before every decade, pinned by test). The original findings below are
kept as history.

### §E1 — original audit (was PARTIAL)
...
`docs/interpretation-guide.md` now exists, with coverage-on-both-bases as
the toggle's second act — the row's experience obligation. The stale
Reckoning copy is fixed (the card now STATES the forced-secondary count
instead of promising the institutional plane). The card metric
(coverage-at-worst) stays consider-not-commit behind I5, as ratified — a
recorded deferral, not an open obligation.

### §E3 — original audit (was OPEN, engine side done)
...
### §E4 — CLOSED 2026-08-12 (sp-03, merge `23ab335`)

Both named annotations exist in `ah/annotations.py`, computed from the
session record alone at outcome time: the flinch cost (one window's
commitments restored to plan; the distribution shortfall stated) and the
arithmetic warning (denominator-driven coverage rise + a negative
chain-link contribution). Tone pinned by test — no judgement words. The
review renders them under "Two things the numbers noticed." The E1
dependency noted below resolved first (sp-01/02).

### §E4 — original audit (was PARTIAL)
