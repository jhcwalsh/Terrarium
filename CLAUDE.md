...
- **`generator_id` is pinned by an enum in `schemas/`** and cannot gain new values. When the
  ...
  **Where the blocks stand now** (`toy-v0.7`, `src/ah/core/engine.py`): toy presets in the
  ...
  and the declared-stress family in the `72x` block (`721` stress_1974_successor, `722` The
  Gulf Decade, `723` stress_1990_successor — the chosen-PE successors, D-ER16-1). Play alphas
...
- **Retired research worlds refuse rebuild by design.** `ah.cli.RETIRED_WORLD_IDS` fences the
  ...
  under the same id, not reproduce the campaign. The fence also holds `711`/`712`/`713`, the
  pre-chosen-PE stress family retired at the v1.3 adoption (D-ER16-1) — those specs live on
