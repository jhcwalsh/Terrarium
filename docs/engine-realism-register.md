...
## ER-16 — Private equity cannot fall harder than its average beta, and was paid an 8%/yr premium while it fell
...
`docs/superpowers/specs/2026-08-19-pe-serenity-finding.md`.

**Scope note.** This is about the GENERATED plane's private-market return
process — `ah/port/adapter.py::_pm_true_monthly_path` reading the sealed
sleeve-mappings artifact — not the `toy-v0` engine. No `TOY_ENGINE_VERSION`
change; the affected stamp is the generated play-alpha version. The toy
plane has its own version of the first half (`pe = 1.4*eq + …`, a constant
1.4) and, unlike `re` and `pc`, carries no crisis term for PE at all.

**What happens.** The generated plane builds PE as an affine function of the
factor path (v1.2's measured row):
...
There is no `max(·,0)`, no regime term, no interaction, no leverage scaling.
A linear map has one beta, so **the crisis beta is the calm beta** — the
model cannot make PE fall harder in a collapse than in a correction, by
construction. Pooled over 24,000 monthly observations per world, fitting
...
CHANGE; removing it leaves zero. **The model's whole answer to a credit
crisis is about -0.8pp of PE return in a -20% equity month.** `hy_spread` —
DN-5's financing channel for buyout — is a declared `structural_omission` in
the sealed artifact, so there is no funding-stress channel of any kind.

**A second term, layered on it, and the larger contributor.** The measured
row's intercept `alpha_quarterly = 0.019441` is applied as `alpha/3` every
month: `(1+0.019441/3)^12 - 1 =` **8.06%/yr**, state-independent. Measured
inside each path's WORST rolling 12-month equity window, 200 paths x 3
worlds, the alpha term contributes **+8.06%** at the median, at p05 AND at
p95 — a constant, paid at full rate through a collapse. On The Gulf
Decade's own live tape the intercept alone compounds to **+117.09%** over
the decade, against the equity market's +107.51%; in a world dialled to
worst-decile severity, PE's decade return beats the equity market on
169/200, 169/200 and 180/200 paths across the three stress worlds.

**Why the measured values could not simply be re-measured better — the
Route-C measurement** (`scripts/pe_desmooth_c_estimation.py`,
`docs/superpowers/specs/2026-08-19-pe-desmooth-c-measurement.md`, run
before any ruling so the decision would rest on measurement, not hope). The
state-dependent smoothing mechanism is real: calm-quarter marks are
essentially honest, stress-quarter marks pass only 55% of reality through
(buyout-native stickiness 0.45, matching the sealed pool's 0.4508). But
pushing that de-smoother through the whole pipeline moves almost nothing
that matters: the reconstructed GFC drawdown deepens only **-26.05% ->
-29.30%** (the one external anchor — secondaries at 0.60 of NAV, itself
flagged illustrative — implies roughly -55%); the refit equity beta **does
not rise** (0.8362 -> 0.8144 fitted / 0.8483 transferred); the intercept
**rises** (8.06 -> 8.65 / 8.52%/yr); and the D-preview finds **no downside
kink on any reconstruction** (|t| < 0.8), so an asymmetric refit after it
would find nothing either. The reason is plain: the index's worst GFC
quarter is -15.01% and its whole GFC peak-to-trough -25.61% — **the crash
was never written down, and no operator fitted on the index can recover
information it does not contain.** No internal fix is supportable.

**The partial mitigation — D-ER16-1 (2026-08-19, owner-ratified).** The row
moves to CHOSEN coefficients with external anchors
(`mappings/sleeve-mappings-v1.3.yaml`, generated by
`scripts/make_sleeve_mappings_v1_3.py`, sealed under `AM-2026-08-19-001`):
`equity_mkt` 1.2 (DN-5's levered-beta prior 1.1-1.3, mid-range — an anchor
that predates every fit) and alpha 3%/yr (`alpha_quarterly` 0.007399; the
marks-free cashflow/PME literature's 2-4%/yr, mid-range). This fixes:
- **the LEVEL** — the always-on premium drops 8.06 -> 3%/yr, so the
  intercept no longer out-earns the equity market of a stress decade; and
- **the crash MAGNITUDE** — beta 0.8362 -> 1.2, so PE now falls *harder*
  than the equity market in a crash, as a levered equity claim should.

It does NOT fix **the missing convexity**: the row is still affine, so the
crisis beta still equals the calm beta — now at 1.2 rather than 0.84. A
levered portfolio's real crash behaviour (funding stress, covenant
pressure, exit markets closing) steepens the response *in the crash
specifically*; nothing in the chosen row can do that, and this entry stays
open for exactly that reason. `r2_train_val` is null on the chosen row —
it is not a fit, and pretending an R2 would dress choice up as measurement.

**Why this is not a defect against the plan.** DN-5 sealed asymmetric betas
out in advance (SM-5: *"Out of v0; in scope for WP3.2 with its own gate"*)
and §7 states the consequence directly: *"The generator is scored on tail
fidelity in sleeve space, using a mapping that cannot produce tails."* This
entry records what that costs on shipped stress worlds, which no document
previously did.

**Composition with the reported plane.** The shipped appraisal filter
(ER-11, `w = 0.35`, unconditional) takes a further ~60% off what does fall:
PE reported max drawdown was **-12.46%** on The Gulf Decade against -30.09%
true and -50.93% equity. DN-5 §5.3 says a constant kernel is the wrong
shape in exactly this state, so the two effects run the same way. The
chosen beta deepens the true plane; the reported plane's constant filter is
untouched by D-ER16-1 and still blurs a crash like a boom.

**What a full fix looks like.** A downside-beta or state-dependent term
needs data the repo does not hold: the Route-C measurement establishes that
no series in the catalog can identify it (the owner rejected
Burgiss/Cambridge composites as same-material appraisal data — a second
smoothed index reproduces the defect). The open route is crisis-visible
external evidence (listed-PE, real secondaries pricing), then SM-5's own
gated refit. Until then the chosen row is the honest posture: right level,
right crash magnitude, stated provenance, no fabricated asymmetry.

**Consequences — what the fix invalidates.** Adopting v1.3 (the NEXT task,
...
record; the chosen row supersedes it forward-only.
