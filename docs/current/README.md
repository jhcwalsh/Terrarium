...
| `private-markets-and-inflation.md` | GOVERNING | How the private market asset classes are modeled across all three layers, and the measured finding that inflation does not reach any of them — PE is bit-identical from 1% to 12% inflation. The finding is filed as **ER-14** in `docs/engine-realism-register.md`, which governs it; this is the supporting detail and the probes. **Served by the hub** (`private-markets-inflation`), with a committed `.pdf` mirror rebuilt by `scripts/build_doc_pdf.py`. |
