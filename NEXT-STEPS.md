# NEXT-STEPS.md — after the ER-14 release (2026-08-19)
...
- **Release 1 — `712f96d`, 2026-08-19: ER-14 closed.** Inflation reaches the
  whole book. Four mechanisms — real estate income escalation vs cap-rate
  repricing, private equity nominal earnings vs multiple compression, private
  credit floating coupon vs coverage squeeze vs convex loss — plus **`infra`, a
  ...
  (−1.123 pp/yr over the 1%→12% probe) where it used to be bit-identical, RE
...
- **Release 2 — `9c15b2b`, 2026-08-19: the picker has worlds again.** Retiring
  ...
  decade rather than one quarter — the ER-14 demonstration world), **`713`
  ...
  **`AM-DSP11-2026-08-19-001`**, re-recording two sealed hashes over canonical LF
  ...
  `3088 passed, EXIT: 0`.
- **Three papers are drafted** and now committed under `docs/papers/` — the
...
- **The stage-2 campaigns are closed, and the coupled engine is UNPROMOTED.**
  ...
  conditioning-reach fix took story↔market agreement from 1.4 to 17.3 points over
  ...
  player-facing.
- **Ranked play stays parked** (D-SP-7, 2026-08-16) — the surface is
...
1. **Adopt the conditional era-crossing rule, or keep it as a measured
   ...
   All 104 crossings were faithful, zero unlicensed. It buys reach 77.6% → 80.7%
   and story/market agreement 77.8% → 78.6% — **and every flesh bar pays for it**:
   `A1` −7.51 → −9.68, `A2`'s high-inflation correlation +0.077 → +0.026, `R2`'s
   p95 1.102 → 1.210, `S1`'s seam p95 2.04 → 2.18. The scoreboard count does not
...
2. **Do `S1` and `A1R` become bars — a fourteen-bar exam?** Both are sealed
...
3. **Is join-selection-by-inflation-distance funded?** `S1` fails on every engine
   ...
   far inflation moves. A compiler that *preferred small-Δ joins* would move
...
4. **`R2`'s p95 half and `S1`'s seam half now overlap badly — split the promise,
   ...
   an explicit seam-frequency statement — or keep it, on the grounds that a
...
measurement (the pooled truth is **−1.54 pp where the sealed single-batch
reading was −7.51 — off by 47 standard errors**, so every prior `A1` verdict in
...
for by this campaign** — see WP-C below.

## 2. The three papers — owner edits
...
## 3. Remote-branch pruning — outward-facing, so it is the owner's call
...
That is the complete list — `origin/main` is the only other remote ref.

**Local, for the same sweep** (not outward-facing, but it is the owner's tree):
twenty local branches are merged into `main` — `app-open-01-fixes`,
...
`Terrarium-spine2`) — removing any is safe only once the owner is done with it.
...
- **Toy-engine realism family** — ER-2, ER-5, the ER-8 remainder and ER-9
  (`docs/engine-realism-register.md`). Deprioritized; decides whether toy presets
  are ever player-facing.
- **ER-15 remains open** — an entered opening book can sit arbitrarily far
  ...
  of ladder shapes would move DN-5 §2.1's sealed pacing figures, which is an
...
## WP-A — the disclosure surface ("how this world was made")
...
decade is prescribed and the months are real — the world card carrying its
...
## WP-B — the entered-book flow, finished
...
that cannot name the world's full private set — deliberate and documented in
...
the derived path got in `app-open-02`. ER-15 stays open regardless — this WP
makes the demotion visible, it does not extend the fitted evidence.

## WP-C — promoting the stage-2 engine into `src/`
...
touched `src/ah/gen/spine.py`. Promotion is a release event in its own right —
...
- **JST commercial use.** `docs/data/LICENSE-REGISTRY.md` states plainly that
  Jordà-Schularick-Taylor is CC BY-NC-SA 4.0 and that commercial providers are
  *"strictly forbidden to integrate all or parts of the dataset into their
  ...
  `**License:** FREE (cite the JST papers)` — the per-source note never received
...
- **`scripts/gen_bundle_fixtures.py` is not reproducible.** Its own docstring
  ...
  under Windows `autocrlf` — and see CLAUDE.md's new seal-EOL gotcha, which is
...
## Resolved since the last revision — dropped from the board

Recorded once, so nobody re-opens them from an old copy.

- **ER-14** — CLOSED and released (above). It was the largest item on this board.
- **F5 (calibration drift)** — closed, batched into the same reseal as ER-14:
...
- **The spine line (D-SP-4 / -6 / -8)** — closed at its second frontier and
  succeeded by stage 2; the spine v2 exam rulings it was waiting on were taken
  and sealed.
- **World 703's precedent line** — the carried follow-up asked whether it
...
- **The local branch sweep and the probe worktree** — both done; the current
