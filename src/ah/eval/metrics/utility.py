...
- :func:`discriminative_score` -- ``|test accuracy - 0.5|`` of a logistic-regression
  classifier trained to distinguish [mean, std] window features of real vs. generated
  factor dynamics. 0 = indistinguishable; lower is better.
...
def _fit_gd(X: np.ndarray, y: np.ndarray, *, loss: str) -> np.ndarray:
    ...
    ``gradient = X^T (pred - y) / n + L2 * w`` form (L2 applied to every weight
    including the bias -- a stated simplification, not excluding the bias from the
    penalty the way some conventions do). Fixed :data:`_GD_EPOCHS` /
    :data:`_GD_LEARNING_RATE` / :data:`_GD_L2`. Pure and seed-free: every call with the
    same ``(X, y, loss)`` returns the bit-identical weights -- see the module
    docstring for why all randomness in this suite lives outside this function.
    """
    ...
    for _ in range(_GD_EPOCHS):
        z = design @ weights
        pred = _sigmoid(z) if loss == "logistic" else z
        gradient = design.T @ (pred - y) / n + _GD_L2 * weights
        weights = weights - _GD_LEARNING_RATE * gradient
    return weights
...
def discriminative_score(
    ...
    *,
    ...
    """``|test-set accuracy - 0.5|`` of a logistic-regression real-vs-generated classifier.
    ...
    std]`` per window -- see :func:`build_utility_suite`). Labels: real=1,
    generated=0. The pooled, labelled example set is shuffled and split
    ``train_fraction``/``1-train_fraction`` via
    ``numpy.random.Generator(PCG64(seed))``; features are standardized (z-score) using
    the TRAIN split's own mean/std only (never the test split's -- a leakage channel
    in the feature scaling itself, distinct from but analogous to the platform's
    train+validation/holdout leakage guard); :func:`_fit_gd` (``loss="logistic"``) is
    fit on the standardized train split; accuracy is measured on the standardized test
    split at a 0.5 probability threshold.

    0 = indistinguishable (the generator fools the classifier); higher = more
    detectably different. **Lower is better** as a G2 criterion.

    ``NaN`` if fewer than 4 pooled examples, or if the resulting test split is empty.
    """
    ...
    n_train = min(max(2, round(train_fraction * n)), n - 2)
    if n_train < 2:
        return float("nan")
    ...
    weights = _fit_gd(X_train_std, y_train, loss="logistic")
...
def tstr_degradation(
    ...
    *,
    ...
    half of the REAL pairs (via the SAME ``numpy.random.Generator(PCG64(seed))`` draw
    predictive_score's subsample uses -- one seed drives every stochastic element of
    one call) and evaluates mean squared error on the other half.
