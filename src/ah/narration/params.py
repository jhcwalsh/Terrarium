...
PARAMETERS: tuple[Param, ...] = (
    ...
    Param(
        ...
        trade_off=(
            "'surprise_scaled' derives dispersion from |surprise_sd| in the quarter, so it is "
            "a function of revealed state and cannot leak; 'regime_conditional' keys off the "
            "L2 label and would make dispersion a regime readout, which is exactly the "
            "non-injectivity failure §3.1 warns about; 'fixed' abandons the mechanic."
        ),
        register="A3-B, N-e",
    ),
    Param(
    ...
    Param(
        ...
        trade_off=(
            ...
            "jitter. EWMA is the cheap filter and has the right serial structure with the "
            "wrong uncertainty. This is a Quant spec, flagged for Counsel-free review."
        ),
        register="A3-B, N-s",
    ),
    Param(
