...
| S2-ENSEMBLE-SIZE | The ensemble size the gate bounds are calibrated at | CLOSED | **The owner directed that a specific `n_paths` be sealed.** Every acceptance band is the sampling distribution of a statistic on ONE length-matched series, while the ensemble side averages over `n_paths` paths — so the gates' *power* rises without limit in ensemble size and `max: 0.5` at 16 paths is not the same criterion as `max: 0.5` at 1000. Sealed: `n_paths: 1000`, `months: 120`, justified by a measured MC-error/band-width analysis on the campaign vintage (worst ratio 0.039 at n≈1024 vs 0.149 at n=64). A run at any other size is recorded `criterion_bearing: false` on its `BatteryReport`, may not be cited in `G2-EVIDENCE.md`, and is rejected by `ah/eval/g2.py`; changing the campaign size is a dated `protocol_change` amendment. **Downstream constraint: WP2.4 and WP2.8–2.10 must produce criterion-bearing ensembles at exactly this size, or amend.** | WP2.4, WP2.8–2.10 |
...
| S2-NUMERAIRE-BIAS | RFR-12's uncommitted-capital bias | CLOSED | Option (b) — seal as-is and record the bias — and **not as a preference**: option (a), adding a `cash_tr_1m` residual leg, would have made `eqw_factors`, `endowment_proxy` and `momentum` *uncomputable*, because `cash_tr_1m` derives from `policy_rate`, which has no data on the sealed vintage. Sealed consequence: those three strategies realize zero on uncommitted/unallocated capital instead of the cash rate, so each sits below the total-return numeraire by that amount. Two are already uncomputable for other reasons; `momentum` is the one live strategy carrying the bias, and its sealed thresholds must be read accordingly. Re-entry when `policy_rate` has data is a `threshold_change` amendment plus recomputed bands. | WP2.4 |
| S2-SEAL-SCOPE-2 | RFR-10 and `battery/thresholds.yaml` | CLOSED | **`ah/data/derive.py`: SEALED** — it is on the read path (`ah.eval.panel._DERIVED_EXPRS` calls `add`/`difference`/`funding_stress` at panel-read time for `equity_mkt`/`ig_spread`/`funding_spread`), so an edit changes what a sealed band is a band *of*. **`ah/data/splice.py`: NOT sealed** — its `PROXY_RULES` are registered but not applied by `ah.data.refresh` and `read_factor_frames` never calls it, which the reference run *demonstrates* (`policy_rate`/`hy_spread` are in `missing_factors` precisely because the backfills are absent). RFR-10 also asked WP2.3 to confirm which the campaign vintage contains: **neither backfill**. **`src/ah/battery/thresholds.yaml`: SEALED** — every entry is `status: todo` and blocks nothing, but it is read by the already-sealed `ah/battery/report.py`, and a sealed estimator over an unsealed input is the exact failure RFR-33 closed for the conditional worlds. Full argument in `pre-registration.yaml`'s `seal_scope:` block. | none |
...
not specify, and its selection-criterion tie-break. Everything else in the file is a
decision rather than an estimate and may be revised only by a further amendment.
